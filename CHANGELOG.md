...
- **`colors.matching: vanilla | perceptual`.** Which formula picks the nearest palette entry to a colour: Minecraft's own weighting, or nearest in Oklab. Two roundings rather than an upgrade - neither measures better, they disagree about some colours - so `vanilla` stays the default and [colour-matching.png](docs/images/colour-matching.png) is the way to pick. Read once at startup.
